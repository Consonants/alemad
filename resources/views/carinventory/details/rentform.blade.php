...
<form method="post" action="{{ url('/details') }}" enctype="multipart/form-data" id="cardetails" class="form-horizontal" id="form-new-post" >
{{ csrf_field() }}
    ...
    <h2>Rent a Car</h2><hr>
        ...
        <div class="col-md-6"> 
            <div class="form-group">
                <label for="inputcompanyname" class="col-sm-3 control-label">Car Make</label>
                <div class="col-sm-6">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-6">
                    {{ Form::select('model', ['' => 'Choose Model'],$users['gearbox'], array('class' => 'form-control','id'=>'model')) }} 
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-6">
                    {{ Form::select('submodel', ['' =>'Choose Sub-Model'],$users['gearbox'], array('class' => 'form-control','id'=>'submodel')) }} 
                </div>
            </div>   
        ...
        <h2 class="headings">Pricing</h2><hr>
        <div class="pricing" id="rent" >
            <div class="form-group">
                <label for="inputcompanyname" class="col-sm-3 control-label">Daily Rent</label>
                <div class="col-sm-6">
                   <input type="text" class="form-control" name="daily" placeholder="Daily Rent" value="{{$users-> daily_rent}}">
                </div>
            </div>
            
            <div class="form-group">
                <label for="inputcompanyname" class="col-sm-3 control-label">Weekly Rent</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" name="weekly" placeholder="Weekly Rent" value="{{$users->monthly_rent}}">
                </div>
            </div>
            
            <div class="form-group">
                <label for="inputcompanyname" class="col-sm-3 control-label">Monthly Rent</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" name="monthly" placeholder="Monthly Rent" value="{{$users->yearly_rent}}">
                </div>
            </div> 
        </div>
        ...
        <div class="form-group">
            ...
            <ul class="checkbox icheck">
                ...
                <li><input type="checkbox" class="offer" name="banner" id="banner" @if(($users->banner)=='1') checked @endif> Banner</li>
                <li><input type="checkbox" class="offer" name="economic" @if(($users->economic)=='1') checked @endif> Economic Cars</li>
            </ul>
            </div>
        </div>
                        
        <div class="col-md-12" id="bannerdivision" style="display:none">
            <h2>Banner </h2><hr>
                <div class="form-group">
                             <label for="inputcompanyname" class="col-sm-3 control-label">Banner Description</label>
                             <div class="col-sm-6">
                            ...
                            <div class="form-group">
                              <label for="confirm" class="col-sm-3 control-label">Upload a Picture</label>
                              <div class="col-sm-6">
                                <div class="dropzone clsbox" id="bannerdropzone"></div>
                                <input type="hidden" name="bannerimage" id="bannerimage">
                              </div>
                            </div>
                            <div class="form-group">
                             <label for="inputcompanyname" class="col-sm-3 control-label">Picture at</label>
                             <div class="col-sm-5">
                                 {{ Form::select('align', ['' => 'Select Car Align','left'=>'Left','right'=>'Right'],$users['type'], array('class' => 'form-control')) }} 
                             </div>
                            </div>
                        </div>
...
$(document).ready(function () {
    ...
    $('.icheck').on('ifChanged', function(event) {
        var banner=$('#banner').is(':checked');
        if(event.target.name=='banner' && event.target.checked==true)
            $('#bannerdivision').css('display','block');
        else if(banner==true && event.target.name!='banner')
            $('#bannerdivision').css('display','block');
        else
...
$("div#bannerdropzone").dropzone({ 
    url: "{{ url('/bannerUploadFile')}} ",
    maxFiles: 1,  
    params: {
        _token: token // dont need this line after following above steps
    },
    success: function(file, response){
        $('#bannerimage').val(response.imageid);
    },
    init: function() {
        this.on("maxfilesexceeded", function(file){
            this.removeFile(file);
            sweetAlert('Sorry!', 'Banner needs a single Image only!', 'warning');
        });
    }  
});
